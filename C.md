...
| `~`    | 按位取反 | 所有位取反           | `~0b1100 = 0b0011`         |
| `<<`   | 左移     | 所有位向左移动       | `0b1100 << 2 = 0b110000`   |
| `>>`   | 右移     | 所有位向右移动       | `0b1100 >> 2 = 0b0011`     |
...
### 字符串函数 len cmp cpy cat chr dup tok 
...
对于指针，**解引用就是根据指针类型的大小，从指针地址开始向后读取相应字节数的内存内容。**
...
int main(void) {
    ...
    printf("int解引用: 0x%x\n", *int_ptr);    
    // 读取 0x1000~0x1003 的4字节 → 0x44332211 (小端序)
