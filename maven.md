...
文章参考：[JavaGuide](https://javaguide.cn/tools/maven/maven-core-concepts.html)
...
- scope(可选)：依赖的范围，默认值是 compile。此外还有，provided, runtime, test, system...
...
| Scope                    | 编译类路径   | 测试类路径   | 运行时类路径 | 是否被打包   | 典型例子                                    |
| :----------------------- | :----------- | :----------- | :----------- | :----------- | :------------------------------------------ |
| **`compile`** (**默认**) | ✅            | ✅            | ✅            | ✅            | `spring-core`, `lombok`, 项目核心依赖       |
| **`provided`**           | ✅            | ✅            | ❌            | ❌            | `servlet-api`, 容器提供的依赖               |
| **`runtime`**            | ❌            | ✅            | ✅            | ✅            | `MySQL JDBC驱动`, 运行时才需要的依赖        |
| **`test`**               | ❌            | ✅            | ❌            | ❌            | `JUnit`, `Mockito`, 仅测试使用的库          |
| **`system`** (不推荐)    | ✅            | ✅            | ✅            | ❌            | 本地系统路径的 JAR，需要配合 `<systemPath>` |
| **`import`** (特殊)      | (不直接使用) | (不直接使用) | (不直接使用) | (不直接使用) | 在 `<dependencyManagement>` 中导入 BOM      |

## 依赖传递

简单来说：**如果你引入了依赖 A，而 A 本身又依赖于 B 和 C，那么 Maven 会自动、递归地把 B 和 C 也引入到你的项目中，而不需要你显式声明。**

**示例：**
假设你的项目直接依赖 `Spring Web MVC`：
...
`spring-webmvc` 本身又依赖 `spring-core`, `spring-beans`, `spring-context` 等一大堆库。通过依赖传递，这些**间接依赖**也会被自动下载并添加到你的项目类路径中。
