...
- **临界区**
    ...
    - 任何解决方案必须满足三个条件：
        1. **互斥**：任何时候最多只有一个进程/线程在其相关的临界区内执行。
        2. **进展**：不在临界区内执行的进程/线程不能阻止其他进程/线程进入临界区。
        3. **有限等待**：一个进程/线程不应无限期地等待进入临界区（避免饿死）。
...
- **硬件解决方案**
    ...
    - **原子指令**：CPU提供特殊的原子指令（如 **TSL (Test-and-Set-Lock)**、**SWAP**、**Compare-and-Swap**）来实现锁。这些是构建更高级同步原语的基础。
...
*   **死锁的四个必要条件**（必须同时满足才会发生死锁）：
    ...
    3.  **不可抢占**：资源只能由持有它的进程自愿释放，操作系统不能强行抢占。
...
*   **处理死锁的三种策略**
    1.  **死锁预防**：通过协议确保**系统永远不会进入**死锁状态。（破坏四个必要条件之一）
    2.  **死锁避免**：系统在分配资源时**动态检查**，确保不会进入**不安全状态**（可能导致死锁的状态）。
...
*   **死锁检测与恢复**
    *   **检测**：
        *   **单实例资源**：使用**等待图**（从资源分配图中简化而来）。如果等待图中存在**环**，则存在死锁。
        *   **多实例资源**：使用类似银行家算法的检测算法。
    *   **恢复**：（非常困难）
        *   **进程终止**：终止所有死锁进程，或逐个终止直到死锁解除。
        *   **资源抢占**：从一个或多个死锁进程中抢占资源，分配给其他进程。需要**回滚**被抢占的进程到某个安全状态，并重启。
...
*   **术语（调度标准）**
    ...
    *   **等待时间**：进程在就绪队列中等待的总时间。
    *   **周转时间**：从进程提交到完成所经历的总时间（等待时间 + 执行时间）。
...
*   **页表的硬件实现**
    ...
    *   每次内存访问理论上需要两次内存访问（一次查页表，一次取数据），**性能极差**。
    *   **TLB**：**转换后备缓冲器**，一个缓存最近使用过的页表项的高速硬件缓存。极大提高了地址转换速度。
...
*   **页面置换算法**
    ...
    *   目标：获得**最低的页错误率**。
    *   评价方法：给定一个**访问串**，计算在不同页框数下的页错误次数。
    *   **FIFO**：置换**最先进入**内存的页面。简单，但性能可能不好（可能出现 **Belady 异常**：页框数增加，页错误率反而升高）。
    *   **LRU**：置换**最久未使用**的页面。基于**局部性原理**，性能很好，是近似最优算法。
...
*   **全虚拟化**
    *   **客户操作系统 unaware** 自己运行在虚拟化环境中，认为自己直接与硬件对话。
...
*   **硬件辅助虚拟化**
    *   一种全虚拟化，但CPU架构提供了特殊指令（如 Intel VT-x, AMD-V）来辅助虚拟化。
...
*   **半虚拟化**
    *   **客户操作系统 aware** 自己是客户，并**直接与主机 OS/Hypervisor 对话**（通过称为 **hypercall** 的特殊调用），而不是与模拟的硬件对话。
...
*   **RAID 核心概念**
    ...
    *   **条带集**：跨多个磁盘的相同位置的一组条带。
    *   **粒度**：
        *   **细粒度**（位/字节级）：高传输速率，但一次只能处理一个请求。
        *   **粗粒度**（块级）：可能将整个文件放在一个磁盘上，允许同时处理多个请求。
    *   **可靠性悖论**：磁盘数量增加会降低平均无故障时间（MTTF），但通过冗余可以弥补。
    *   **磁盘镜像**：一个磁盘是另一个磁盘的完整副本。实现简单，但存储开销大（100%）。
    *   **RAID 控制器**：专用于 RAID 操作的硬件，减轻 CPU/OS 负担，但可能昂贵。

*   **RAID 级别**
    *   **RAID 0**：**条带化**。将数据块拆分到多个磁盘。**性能高**，但**无冗余**。任何一个磁盘故障都会导致所有数据丢失。
    *   **RAID 1**：**镜像**。数据同时写入两个磁盘。**读性能好**（可从任一磁盘读），**可靠性高**，但**存储开销大**（利用率 50%）。
    *   **RAID 2**：**位级条带化** + **汉明码**纠错。由于现代硬盘已内置纠错，此方案**冗余且复杂**，**无商业应用**。
    *   **RAID 3**：**字节级条带化** + **专用奇偶校验盘**。需要磁盘同步旋转。**非常适合大文件顺序读写**（如视频、卫星图像）。
    *   **RAID 4**：**块级条带化** + **专用奇偶校验盘**。与 RAID 3 类似，但条带单位是块而非字节。
    *   **RAID 5**：**块级条带化** + **分布式奇偶校验**。奇偶校验信息均匀分布在所有磁盘上，而非专用盘。**性能、成本、可靠性取得了良好平衡**，被广泛使用。
    *   **RAID 6**：**块级条带化** + **双重分布式奇偶校验**。可容忍**两个磁盘同时故障**。读操作无性能 penalty，但写操作有开销。
...
*   **分布式系统中的身份验证：**
    ...
    *   **“密钥对”方法**：使用**非对称加密**（公钥/私钥）来验证身份和确保通信安全。
        *   **私钥 (Private Key)**：仅自己知道，用于加密或签名。
        *   **公钥 (Public Key)**：公开可用，用于解密或验证签名。
...
*   **核心原则：**
    *   **最小权限原则 (Principle of Least Privilege)**：只授予程序、用户和系统执行任务所必需的**权限 (Privileges)**。
