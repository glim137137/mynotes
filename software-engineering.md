...
- **Three Phases in Scrum**
    ...
    - **初始规划阶段：** 确定项目总体目标与软件架构，建立产品待办列表。
    - **冲刺循环阶段：** 进行一系列固定周期的迭代开发，每个冲刺交付一个可用的软件增量。
    - **项目收尾阶段：** 完成文档、用户手册并进行项目总结。
...
- **Agile methods for large systems**
    ...
    **2. 棕地开发与增量限制**
    ...
    - **棕地系统**：绝大多数大型系统是“**棕地系统**”，即它们需要与大量现有（遗留）系统进行交互和集成。
    ...
    **5. 项目周期与团队延续性**
    ...
    - **知识流失**：在漫长的周期中，很难维持一个了解系统全貌的、稳定的核心团队。人员的流动不可避免地会导致**系统知识的严重流失**。

    **6. 利益相关者管理困难**

    - **利益相关者众多且多样**：大型系统通常涉及大量不同背景和诉求的利益相关者（如用户、客户、监管机构、运维团队等）。
...
- **Types of requirements**
    ...
    2. 系统需求
        - **定义**： 一个结构严谨、内容详细的文档，精确定义系统“必须实现什么”。它是开发团队的技术蓝图，甚至可以成为法律合同的一部分。
...
- **Interviewing**
    ...
    - **类型**：
        - **封闭式访谈**： 基于预先准备好的问题清单进行，效率高，但可能限制思路。
        - **开放式访谈**： 没有固定结构，围绕几个核心话题自由交流，有助于发现深层和未预料到的需求。
    - **技巧**：
        - **保持开放心态**： 不要预先假设答案，认真倾听。
        - **有效引导**： 使用“弹簧板问题”（例如：“如果……你会怎么想？”）、需求提案或原型来激发讨论。
...
- **Requirements documentation**
    ...
    | **用户需求定义**     | 用非技术语言描述服务和非功能需求，面向客户。   |
    | **系统架构**         | 高层架构图，展示模块划分。                     |
    | **系统需求规格说明** | **核心章节**，详细描述功能和性能等非功能需求。 |
    | **系统模型**         | 放置各种UML图（用例图、类图等）。              |
...
**上下文模型**用于说明系统的**运行上下文**——它清晰地展示了哪些部分位于**系统边界之外**。
...
- **Process perspective**
    - 上下文模型仅显示环境中的其他系统，而**不展示**待开发系统在该环境中**如何被使用**。
    - 过程模型则揭示了待开发系统在**更广泛的业务流程**中是如何被使用的。
    - **UML活动图 Activity diagrams**可用于定义业务过程模型。
...
- 这些（状态机）模型描述了系统为响应**外部和内部事件**而表现出的行为。
- 它们展示了系统对刺激的响应，因此常被用于为**实时系统**建模。
...
- **How to represent architecture？**
    - 展示实体与关系的**简单、非正式框图**，是记录软件架构最常用的方法。
...
| 适用场景 | 通常用于数据处理应用（包括批处理和基于事务的处理），其中输入在独立阶段被处理以生成相关输出。 |
...
- **设计模型分为两种**：
    - **结构模型**从对象类和关系的角度描述系统的**静态结构**。
    - **动态模型**描述对象之间的**动态交互**。

- **Categroy**
    - **子系统模型**：显示对象如何被**逻辑分组**为内聚的子系统。
    - **序列模型**：显示对象交互的**顺序**。
    - **状态机模型**：显示单个对象如何响应事件而改变其**状态**。
    - **其他模型**还包括用例模型、聚合模型、泛化模型等。
...
- **More Example**
    ...
    - 整理一系列通常以增量方式开发的相关对象的接口**（外观模式）**
    - 提供一种标准方式来访问集合中的元素，而无需关心该集合的具体实现方式**（迭代器模式）**
    - 允许在运行时扩展现有类的功能**（装饰器模式）**
...
- **Reuse**
    - **抽象层次**
        - 在这个层次上，你不是直接复用软件，而是在软件设计中**复用成功抽象的知识**。
    - **对象层次**
        - 在这个层次上，你直接从库中**复用对象**，而不是自己编写代码。
    - **组件层次**
        - **组件**是对象和对象类的集合，你在应用系统中复用这些组件。
    - **系统层次**
        - 在这个层次上，你复用**整个应用系统**。

- **Configuration management**
    - 配置管理是对**不断变化的软件系统**进行管理的通用过程的统称。
    - 配置管理的目标是**支持系统集成过程**，使得所有开发人员能够以**受控的方式**访问项目代码和文档，**查明已进行了哪些更改**，并编译和链接组件以创建系统。
        - **版本管理**：提供支持以**跟踪软件组件的不同版本**。版本管理系统包含**协调多个程序员开发**的功能。
...
- **Open source license models**
    - **GNU通用公共许可证**：这是一种所谓的“**互惠**”许可证，意味着如果你使用了基于GPL许可证的开源软件，那么**你必须将该软件开源**。
    - **GNU宽通用公共许可证**是GPL许可证的一个变体，允许你编写**链接到开源代码的组件**，而**无需发布这些组件的源代码**。
...
- **Inspections and testing**
    ...
    - **软件审查（Software Inspections）**侧重于分析系统的静态表示形式以发现问题（即静态验证）。
        - 可借助基于工具的文档和代码分析作为补充。
        - 第 15 章将对其进行详细探讨。
    - **软件测试（Software Testing）**侧重于运行并观察产品行为（即动态验证）。
...
- **Unit Testing**
    ...
    - 第一类用例应能反映程序的正常运行情况，且需验证组件是否按预期工作。
    - 第二类用例应基于常见问题发生场景的测试经验来设计，需使用异常输入验证组件能否对其进行正确处理，且不会因异常输入导致组件崩溃。
...
- **Box Testing**
    ...
    - **白盒测试（White-box Testing）**
        - 白盒测试通常作为黑盒测试的**补充手段**。开展白盒测试时，需查看程序内部结构，并根据程序所有可能的逻辑流程设计测试用例。测试用例中的程序输入应能覆盖程序所有分支的可能性，因此，需要详细了解程序的内部实现。 all  possible logical flows 
...
- **System testing**
    - 开发过程中的系统测试需先将组件集成以构建一个系统版本，随后对该集成系统进行测试。
    - 系统测试的核心重点是测试组件之间的交互。
...
- 发布测试是系统测试的一种形式，由需求测试、场景测试和性能测试三部分构成。
- 与系统测试的核心区别：
...
- **What are legacy systems?**
    ...
    - 遗留系统并非仅指软件系统，而是范围更广的社会技术系统，其中包含硬件、软件、程序库、其他支撑软件以及业务流程。
...
- **Personality types**
    ...
    - 激励还应考虑不同的人格类型：
        - 任务导向型：受自身所做工作的激励，在软件工程领域较为常见。
        - 互动导向型：受同事的存在及行为的激励。
        - 自我导向型：主要受个人成功与他人认可的激励。
...
> - Planning stages
>
>     - **提案阶段**：在竞标开发或提供软件系统的合同时进行。
>     - **项目启动阶段**：需要规划项目人员安排、项目如何分解为增量、公司内部资源如何分配等。
>     - **项目期间的周期性规划（开发规划）**：根据获得的经验及工作进度监控信息，对计划进行修改。
>
...
>     - 质量管理为软件开发过程提供独立检查。
>     - 质量管理流程会核查项目交付物，确保其符合组织标准与目标。
>     - 质量团队应独立于开发团队，以便对软件形成客观看法。这使其能够不受软件开发相关问题的影响，客观报告软件质量状况。
> - Scope of quality management
>     - 质量管理对大型复杂系统尤为重要。质量文档既是项目进度的记录载体，也能在开发团队人员变动时，为开发工作的连续性提供支持。
>     - 对于小型系统，质量管理所需的文档较少，应重点聚焦于质量文化的构建。
>     - 当采用敏捷开发模式时，质量管理技术必须相应迭代演进。
...
- **Quality conflicts**
    - 任何系统都无法同时对所有质量属性进行优化 —— 例如，提升健壮性可能会导致性能下降。
    - 因此，质量计划需明确所开发软件的核心质量属性（即优先级最高的质量指标）。
    - 该计划还应包含质量评估流程的定义，即一套已达成共识的判定方法，用于评估产品是否具备某项质量属性（如可维护性或健壮性）。

- **Process-based quality**
    - 所开发产品的质量受生产过程质量的影响。
    - 其中，个人技能与经验的运用在软件开发中尤为重要。

- **Quality culture**
    - 质量管理者的核心目标应是构建一种 “质量文化”—— 在这种文化中，所有对软件开发负责的人员均致力于实现高水平的产品质量。
    - 质量管理者应鼓励团队对自身工作质量承担责任，并积极探索质量改进的新方法。
    - 他们还应支持那些关注质量无形价值的人员，并倡导所有团队成员秉持专业行为准则。
